...
class RandomNoise(object):
    ...
    def __call__(self, image, target=None):
        ...
        image = add_noise(image, self.noise_factor)
        if target is None:
            return PIL.Image.fromarray(image), None
        else:
            return PIL.Image.fromarray(image), target
        
class Save(object):
    def __call__(self, image, target=None):
        Path.mkdir(Path('transformed'), parents=True, exist_ok=True)
        save_path = Path('transformed', f'{time.time()}.jpg')
